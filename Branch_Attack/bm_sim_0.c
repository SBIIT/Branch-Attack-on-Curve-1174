...
static void pt_add_after(point_t *pt1, point_t *pt2,point_t *pt3)
{
	...
	mulp(S_1,pt1->x,pt2->y); fprintf(f1,"\n");
	mulp(S_4,pt2->x,pt1->y);fprintf(f1,"\n");
	mulp(S_2,pt1->x,pt2->x);fprintf(f1,"\n");
	mulp(S_3,pt1->y,pt2->y);fprintf(f1,"\n");
	mulp(T_1,pt1->z,pt2->z);fprintf(f1,"\n"); 
	
	
	addp(T_2,S_1,S_4);
	mulp(S_5,T_1,T_1);fprintf(f1,"\n");
	subp(T_3,S_3,S_2);fprintf(f1,"\n");
	mulp(S_6,T_1,T_2);fprintf(f1,"\n");
	mulp(S_7,T_1,T_3); fprintf(f1,"\n");
	mulp(T_4,S_2,S_3);fprintf(f1,"\n");
	mulp(T_9,CURVECONST_D,T_4);fprintf(f1,"\n");
	
	
	copy_element(T_6,S_6);
	copy_element(T_8,S_7);
	subp(T_5,S_5,T_9);fprintf(f1,"\n");
	mulp(X_3,T_5,T_6);fprintf(f1,"\n");
	addp(T_7,S_5,T_9);
	mulp(Y_3,T_7,T_8);fprintf(f1,"\n");
	mulp(Z_3,T_5,T_7);fprintf(f1,"\n");
...
void smul(field_t *k)
{
	...
	for(; j<msb; j++)
	{
		...
		f1 = fopen("for_samples/bmsim_0/bm_save","a");
		//Extra computation with the last bit guessed as 0
			b = 1; 
			pt_add_after(&R[b],&R[b],&q);
			pt_add_after(&R[!b],&q,&R[b]);
...
int main(int argc, char** argv)
{
	...
	f1 = fopen("for_samples/bmsim_0/bm_traces","a");
	//Start simulating
	smul(k);
	//fprintf(f1,"\n");
	fclose(f1);
